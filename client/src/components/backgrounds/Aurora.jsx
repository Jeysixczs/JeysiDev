...
export default function Aurora({
  ...
  useEffect(() => {
    ...
    // Tracks whether the Hero section is actually on screen. The aurora
    // is expensive full-viewport WebGL — once the user scrolls past it,
    // there's no reason to keep painting it every frame, so the RAF loop
    // is gated on this in addition to the `paused` (reduced-motion) prop.
    let isVisible = true;
    ...
    function frame(timestamp) {
      ...
      rafId = !paused && isVisible ? requestAnimationFrame(frame) : null;
    }

    function startLoop() {
      if (rafId !== null || paused || !isVisible) return;
